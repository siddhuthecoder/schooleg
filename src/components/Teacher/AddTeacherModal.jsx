...
const AddTeacherModal = ({ isOpen, onClose }) => {
  const SCHOOL_ID = '60d5ec49e65e9a001f1b1b6a'; // static school ID
  const BRANCH_ID = '60d5ec49e65e9a001f1b1b6c'; // static branch ID
  ...
  const [formData, setFormData] = useState({
    ...
    salary: '',
    photo: null,
    route_id: '60d5ec49e65e9a001f1b1b6b', // Updated to reflect route_id
    address: {
      name: '',
      state: '',
      district: '',
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('Data', JSON.stringify({
      ...
      school_id: SCHOOL_ID,
      branch_id: BRANCH_ID,
      address: formData.address, // Add the address directly as an object
    }));

    // Append photo separately if needed
    if (formData.photo) {
      data.append('Photo', formData.photo);
    }

    try {
      const response = await fetch('https://schooleg.com/Schooleg/teacher', {
        method: 'POST',
        body: data,
      });
      ...
      <div className="bg-white rounded-lg w-3/4 md:w-1/2 lg:w-2/5 p-6 shadow-lg">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="mail" className="block font-medium mb-1 text-left">Email</label>
              <input
                type="email" // Corrected type for email
                name="mail"
                value={formData.mail}
                onChange={handleInputChange}
                placeholder="Email"
            ...
            <div>
              <label htmlFor="designation" className="block font-medium mb-1 text-left">Designation</label>
              <select
                name="designation"
            ...
            <div>
              <label htmlFor="photo" className="block font-medium mb-1 text-left">Photo</label>
              <input
                type="file"
                name="photo"
            ...
            <div>
              <label htmlFor="route_id" className="block font-medium mb-1 text-left">Route</label>
              <select
                name="route_id" // Updated to reflect route_id
              ...
              >
                ...
                <option value="60d5ec49e65e9a001f1b1b6b">Route 1</option> {/* Use IDs here */}
                <option value="60d5ec49e65e9a001f1b1b6b">Route 2</option>
                <option value="60d5ec49e65e9a001f1b1b6b">Route 3</option>
              </select>
            </div>
